...
**上线前必须将 `database_id` 占位符 `00000000-0000-0000-0000-000000000001` 替换为真实 D1 UUID**（`wrangler.cron.jsonc`、`wrangler.backup-cron.jsonc` 同步更新）。
...
| 配置 | Worker 名称 | 调度 (UTC) | 功能 |
|------|-------------|------------|------|
| `wrangler.cron.jsonc` | `crm-system-reclamation-cron` | `0 5 * * *`（每日 05:00） | 客户自动回收 |
| `wrangler.backup-cron.jsonc` | `crm-system-backup-cron` | `0 21 * * *`（每日 21:00） | 自动数据库备份 |
...
| Staff 无法导入导出备份等 | ✅ API + 页面级权限；见权限测试清单 |
| Turnstile | ⚠️ **未启用**（已标注） |
...
| 项 | 说明 |
|----|------|
| `/import`、`/export` 未列入 `proxy.ts` matcher | 页面 layout 与 API 仍有 Admin 守卫；Staff 直链 URL 会得到 403 页面而非重定向 |
...
- Cron：`crm-system-backup-cron`，每日 21:00 UTC
