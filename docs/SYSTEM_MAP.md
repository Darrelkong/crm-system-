...
|------|-----|
| 分支 | `main` |
| 最新 commit | `8f98634` — Clarify public pool quota help text |
| Production Worker | `crm-system` @ `crm.echfronthk.com` |
| 最新 Production Version ID | `83be19d0-14e5-4bdf-802a-8be4e4ac96ce` |
...
| `/admin/tags-stages` | 客戶標籤管理 + 銷售階段分布 | ✅ 已完成 | i18n `placeholders.tagsStagesEmpty` 仍寫「即將推出」（文案過時） |
...
| **回收站** | ✅ | Admin | Purge 後 tasks 設 `customerId=null`；notifications 可能 orphan | 見下方「已知風險」 |
...
**實作參考：** `src/lib/permissions/customers.ts`、`src/lib/permissions/auth.ts`
...
| 🟡 | **Recycle purge 關聯資料 orphan** | 永久刪除 / cron purge 後：`tasks.customerId` 設為 `null`（保留 open task）；`notifications` 仍指向已刪 customerId | 測試已文件化於 `purge-relations.test.ts`；UI 可能見到孤立通知 |
...
| 🟢 | **Stale i18n placeholder** | tags-stages「即將推出」文案與實際 UI 不符 | 下個 i18n 清理 PR |
