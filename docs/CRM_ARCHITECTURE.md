...
Incremental candidate materialization preserves existing candidate state and uses a conditional insert with segment uniqueness. Source/segment/analysis identifiers must agree; the segment must remain confirmed in the latest completed analysis, with no newer analysis, and the source must remain unarchived and eligible. These conditions are re-evaluated by SQL when a mutation commits, not only by an earlier DTO read.

**1B-A local remediation — 2026-09-26 (uncommitted diff based on `86c33c4c5e3a0a8ce91a167ed11407d42d44cd24`):**

- Candidate PATCH authorizes the actor/source/lineage, validates the entire request, then performs one conditional update. Denied or invalid PATCH requests do not partially update business/category fields.
- Candidate conversion uses one D1 batch: conditional Article insert, version 1, create audit and candidate linkage/time. Dependent writes require this request's newly created Article ID; a competing request inserts nothing and reads the canonical linkage. A batch error after a possible commit recovers through that same linkage. No historical duplicates are deleted.
- Re-analysis atomically checks for any converted candidate, creates its run, supersedes old segments/candidates and changes source analysis state. A competing conversion and re-analysis cannot both commit incompatible transitions.
- Organizer/comparison completion checks current lineage at commit. Candidate comparison requires the browser draft's organization run ID; conversion also requires the latest completed candidate organization/comparison and matching draft fingerprint. Reorganization preserves an already-linked Article; idempotent conversion returns it rather than creating a replacement.
- Automatic classification uses a candidate revision CAS, manual override checks and live mapping/category activity. Late AI/mapping output cannot overwrite a newer human edit. Medium/low/error clears a prior automatic fill when the captured revision remains current.
- 1B-A3 adds the explicit `restoreAutomaticClassification: true` Candidate PATCH action. A manual category selection or clear always records manual mode (including null); only this reset releases it. Reset and live mapping resolution share the existing guarded UPDATE/revision change. Organizer hydration cannot use a false request flag to bypass persisted manual mode. The reset UI rehydrates category suggestions without replacing the user's edited article text.
- Legacy/source-level organization lookups in comparison and review explicitly require `candidate_id IS NULL`; candidate paths remain candidate-scoped. The source archive guard intentionally checks **all** active organization runs because either scope can prevent archive.

The earlier split-write conversion risk at `d9e94c37fb1af503a116663b8da667db4d7fa6dd` was classified as blocking in 1A. The local remediation requires Chat review and 1B-B full validation; it does not change 1A's recorded outcome to PASS. Schema/migrations remain through 0090 unchanged. See [validation evidence](CRM_MODULE_STATUS.md#1b-a-local-remediation-evidence) and [P1-01](CRM_IMPLEMENTATION_PLAN.md#p1-01--candidate-conversion-concurrency). **NOT PRODUCTION DEPLOYED**.
