...
| Knowledge Preview | `npm run deploy:knowledge-preview -- --sha <approved-feature-SHA>` | Approved SI2 Preview only; exact pushed SHA and strict isolated binding guard; real AI, Mail disabled. Browser Access remains required. |
...
The authorized B2R target uses Worker `crm-system-knowledge-preview`, D1 `crm-db-si2-preview` / `66f0e690-25d1-45d2-aa16-197c5e7f0802`, R2 `crm-knowledge-si2-preview`, and service `crm-ai-si2-preview`. The existing hostname `knowledge-preview.echfronthk.com` remains behind its unchanged Access policy. Old `crm-db-knowledge-preview` and `crm-knowledge-sources-preview` are preserved; neither is a deployment target.

[Deploy guard](../scripts/deploy-knowledge-preview.mjs) requires `feat/knowledge-smart-ingest-2`, a clean worktree, and equality of local HEAD, the explicit `--sha` argument and remote feature HEAD. It rejects unexpected binding types, service environment overrides, old/Production resources and unsafe flags. Mock AI and test DB fallback are off; notification verification/outbound Mail and large attachment runtime/send are disabled. Only the existing Preview custom domain is permitted; workers.dev, preview URLs, cron and email bindings are disabled. Use `npm run deploy:knowledge-preview -- --sha <approved-feature-SHA> --check` before deployment. The script rechecks identity after building and tags the Worker version with the SHA.

[Preview AI configuration](../workers/crm-ai/wrangler.si2-preview.jsonc) deploys only `crm-ai-si2-preview`, with Workers AI as its only resource binding and no public routes. `CRM_AI_GATEWAY_MODE=direct` omits Gateway options; unset/default configuration retains Production's existing `default` gateway behavior. No shared Gateway change is required. Before an explicitly approved Preview AI deployment, run `npm run crm-ai:typecheck`, the relevant Worker tests, and repository-installed Wrangler `deploy --dry-run --config workers/crm-ai/wrangler.si2-preview.jsonc`; deploy with that exact config and `--tag <approved-feature-SHA>`. Read back both Workers' active-version bindings after deployment before acceptance.

Fresh Preview D1 preparation applies repository migrations 0001–0086 sequentially, creates a small synthetic legacy Knowledge baseline, then applies 0087–0090 individually. Check tracking, schema, FK/quick_check and baseline preservation at each stage. Never use root `db:migrate:remote` for Preview. All SQL must explicitly select the new Preview database/config. Production migrations and old Preview resources remain out of scope.

The build retains `NEXT_PUBLIC_MAIL_READ_SOURCE=preview` (prototype Mail); Knowledge acceptance does not verify Production Mail. Engineering service checks may use authenticated Wrangler bindings with synthetic data only. They do not establish HTTP authentication or browser acceptance. With the owner away, browser ingest, cards, manual controls/reset, Open Draft, reload, permission UX and stale-state UX remain **DEFERRED — HUMAN ACCESS REQUIRED**. Never weaken or bypass Access. Full B2 PASS requires those later checks.

Rollback is limited to the new Preview targets: stop testing and retain the isolated resources; redeploy a reviewed compatible Preview version/config only under the owner's applicable authorization. Do not restore the historical Production AI binding, delete old Preview resources, or roll back Production. Real Workers AI calls can consume account quota/billable usage; keep synthetic fixtures and call counts small.
