...
| 命令 | 說明 |
|------|------|
| `npm run test:unit` | 32 個純單元測試（無 D1；含 `backup/export-data.test.ts` 表覆蓋） |
| `npm run test:db` | 16 個 local D1 整合測試（固定 `CRM_ALLOW_TEST_DB_BIND=1`、`--test-concurrency=1`；含 backup export 整合） |
| `npm run test` | `test:unit` + `test:db`（全部 48 個 `*.test.ts`） |
...
| 類型 | 數量 | 目錄範例 |
|------|------|----------|
| 純單元 | 32 | `help/`, `permissions/`, `auth/`, `reclamation/`, `backup/export-data`, `ai/`（provider、diagnostics、error-mapping、cooldown、prompt-builder、context-sanitize）、`notifications/notification-href` |
| Local D1 | 16 | `public-pool/claim-limits`, `customers/assignees*`, `recycle-bin/*`, `users-admin/delete-*`, `notifications/queries`, `ai/customer-insights/service-cooldown`, `backup/export-data` |
