...
**更新：** Phase RELEASE-FINAL-CHECKPOINT-2026-07（2026-07-01）— 正式站人工 smoke **PASS**  
**先前：** Phase AUDIT-CHECKPOINT-1（2026-07-01）— Admin 審計日誌 API + 只讀 UI 已部署並記錄  
**用途：** 記錄當前穩定版本基線，方便未來回溯、deploy 對照與 rollback 決策。  
**相關：** [SYSTEM_MAP.md](./SYSTEM_MAP.md) · [DEPLOY_RUNBOOK.md](./DEPLOY_RUNBOOK.md) · [BACKUP_RESTORE_RUNBOOK.md](./BACKUP_RESTORE_RUNBOOK.md) · [PRODUCTION_SMOKE_CHECKLIST.md](./PRODUCTION_SMOKE_CHECKLIST.md) · [PRODUCTION_MANUAL_SMOKE_2026_07.md](./PRODUCTION_MANUAL_SMOKE_2026_07.md) · [TESTING.md](./TESTING.md)
...
| 狀態 | **正式站可正常使用**；Admin / Staff 人工 smoke **PASS**（2026-07-01）；Version `537d7889-…` |
...
## R2 backup retention lifecycle

Phase R2-RETENTION-1B Verify + R2-RETENTION-CHECKPOINT-1（2026-07-01）：

| 項目 | 值 |
|------|-----|
| R2 bucket | `crm-attachments` |
| Lifecycle rule name | `backups` |
| Prefix | `backups/` |
| Expiration | **90 days** |
| Default Multipart Abort Rule | **仍保留** — 7 days，`(all prefixes)`；僅中止未完成 multipart upload |
| 整 bucket object expiration | **無** — expiration 僅套用 `backups/` prefix |
| 設定驗證時間 | **2026-07-01** |
| object_count | **8** |
| bucket_size | **1.64 MB** |

**結論：** `backups/` 90 天保留策略**已生效**；現有備份均在 90 天內，**未被刪除**。`backup_jobs` metadata 可能長期保留 `completed` 記錄，即使 R2 物件日後被 lifecycle 清除（見 `BACKUP_RESTORE_RUNBOOK.md`）。

**後續候選：** R2-RETENTION-2 — Admin UI 標記 R2 物件已過期 / 不存在。
...
| Admin OTP 登入後 nav / 列表 / filters / metadata 摺疊 | ✅ **人工 smoke PASS**（2026-07-01） |
| Staff OTP 登入後無 nav / 無法訪問 `/admin/audit-logs` | ✅ **人工 smoke PASS**（2026-07-01） |

**後續候選：** audit export（CSV / JSON）、`audit_logs` retention policy、action 人類可讀 label 優化。

---

## Production manual smoke result

Phase RELEASE-FINAL-CHECKPOINT-2026-07 + [PRODUCTION_MANUAL_SMOKE_2026_07.md](./PRODUCTION_MANUAL_SMOKE_2026_07.md)（2026-07-01）：

| 項目 | 值 |
|------|-----|
| 驗收時間 | **2026-07-01** |
| 驗收人 | 使用者人工確認 |
| 結果 | **Admin / Staff 主要功能均正常，暫未發現問題** |
| 正式站 | https://crm.echfronthk.com |
| Overall | **PASS** |
...
|------|------|
| AI-2 / AI-2g | 已部署 |
| MERGE-SAFE-1 | merge placeholder 已禁用 |
| TAGS-STAGES-CLEANUP-1 | 已部署 |
| BACKUP-EXPORT-1 | scheduled backup `table_count = 21` |
| R2 `backups/` retention | **90 天** lifecycle 已生效 |
| AUDIT-UI-1 | Admin 審計日誌頁已上線 |

### 危險操作（均未執行）

- permanent delete、restore、migration、manual backup、delete production tags、approve merge、excessive AI refresh
...
**2026-07-01 更新：** 正式站人工 smoke 已完成，結果 **PASS**。詳見上方 **Production manual smoke result** 與 [PRODUCTION_MANUAL_SMOKE_2026_07.md](./PRODUCTION_MANUAL_SMOKE_2026_07.md)。

以下為原 checklist 對照（均已人工確認通過）：

| # | 檢查項 | 預期 | 結果 |
|---|--------|------|------|
| M1 | Admin 客戶詳情「提交審批」下拉 | **無**「合併客戶」 | ✅ |
| M2 | Staff 客戶詳情「提交審批」下拉 | **無**「合併客戶」 | ✅ |
| M3 | Approvals 頁 `/approvals` | 頁面正常 | ✅ |
| A1 | Help Center AI 資料範圍 | 三語正常 | ✅ |
| A2 | AI panel | 單次 refresh / cooldown 正常 | ✅ |
| A3 | System Online / customers / public pool / notifications | 基本載入正常 | ✅ |
| AL1 | Admin 審計日誌 nav | 可進入 `/admin/audit-logs` | ✅ |
| AL2 | Audit metadata 摺疊 | 正常 | ✅ |
| AL3 | Audit filters / 載入更多 | 正常 | ✅ |
| AL4 | Staff 無審計日誌入口 | 無 nav / 無法訪問 | ✅ |
...
1. **Audit export / retention / action labels** — 見「Audit logs visibility」後續候選
2. **Backup restore 演練** — runbook 對照（非 production smoke 範圍）
3. **真正 `merge_customers` 完整產品設計** — schema、canonical ID、衝突解決、migration
4. **AI notes / follow-ups 內嵌聯絡方式遮罩設計** — AI-2g-5 候選
5. **AI-2g-4：** 可選 Admin setting 控制 AI 是否可使用敏感聯絡資料
6. **AI-2e / AI-2f：** fallback provider、成本 / token 估算與日限額
7. **Public pool 更完整 API 測試** — claim / list 整合測
8. **UI polish** — 空狀態、loading、錯誤提示一致性
9. **Sales stages Admin CRUD** — 若需可配置階段，需 schema + migration 設計
10. **R2-RETENTION-2** — Admin UI 標記 R2 物件已過期 / 不存在
