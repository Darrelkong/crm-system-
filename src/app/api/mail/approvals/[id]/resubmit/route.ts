...
import { resubmitRevisionForApproval } from "@/lib/mail/outbound-approval-service";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    const revisionId = readStringField(body, "revisionId");
    ...
    if (!revisionId) {
      return Response.json(
        { error: "revisionId is required", errorCode: "VALIDATION" },
        { status: 400 },
      );
    }
    const priority =
    ...
    const item = await resubmitRevisionForApproval(db, actor, {
      approvalId: id,
      revisionId,
